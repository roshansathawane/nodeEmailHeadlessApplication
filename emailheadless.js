...
app.post('/email', async (req, res) => {
  ...
  if (!emailId || !password) {
    return res.status(400).send('Both emailId and password are required.');
  }

  try {
    await emailExtract(emailId, password);
...
async function emailExtract(emailId, password) {
  try {
    ...
    for (let i = 0; i < emailCount; i++) {
      ...
      if (emailElement) {
        ...
        if (boundingBox) {
          ...
          const folder = path.join('S:\\GMAIL-EXTRACTION', currentDate);
